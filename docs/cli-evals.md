...
| grade-page-changed | missing API | broken fixture | `PAGE_CHANGED` |
  ...
  "cases": [
    {
      ...
      "command": "notice list --publisher 土木与交通工程学院 --year 2026 --json",
      "score": 100,
      "checks": {
        "functional": true,
        "robustness": true,
...
1. 先把现有 `tests/*` 映射成 eval case id。
2. 给每个模块补缺的 fixture case，尤其是无结果、页面变化、隐私泄露检测。
3. 给每个 case 标注 `layer: L0/L1/L2` 和 `evaluator`。
4. 增加一个轻量 runner：运行命令、捕获 stdout/stderr/exitCode/duration、执行 L0 检查规则、输出 JSON。
5. 对少量 L1 case 增加 rubric judge。
6. 最后再做 L2 live smoke，只记录 pass/fail 和脱敏摘要。
