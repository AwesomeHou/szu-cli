...
```bash
szu-cli cnki status --headed
szu-cli wanfang status --headed
```
...
| 用户意图 | 使用 | 边界 |
|---|---|---|
| 检索论文 | `cnki search <keyword> --headed` 或 `wanfang search <keyword> --headed` | 仅元数据 |
...
| 单篇详情 | `cnki item <url> --headed` 或 `wanfang item <url> --headed` | 条目命令不下载 |
| 单篇指定全文 | `cnki download <url> --headed --dir <path>` 或相应万方命令 | 仅点击页面可见下载按钮下载单篇 |
