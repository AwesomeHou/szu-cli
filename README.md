...
CNKI and Wanfang commands are headed-browser, read-only metadata MVPs. They use SZU library campus access channels and return search result metadata such as title, authors, source, year, download count, and detail URL. CNKI supports basic fielded advanced search with `--title` and repeatable `--abstract`, for example `cnki search --title 优化 --abstract 交通 --abstract 调度 --headed --json`; this advanced-search MVP defaults to the 学术期刊 database scope. They intentionally do not download PDFs, CAJ files, or original full text. Use `--headed`; headless mode is not reliable for these providers.
